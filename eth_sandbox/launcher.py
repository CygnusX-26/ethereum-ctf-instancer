...
FLAG = os.getenv("FLAG", "bctf{placeholder}")
...
def new_launch_instance_action(
    ...
    def action() -> int:
        ticket = check_ticket(input("team ticket (generate one by running gen-ticket.py): "))
        if not ticket:
...
def new_kill_instance_action():
    def action() -> int:
        ticket = check_ticket(input("team ticket please: "))
        if not ticket:
            print("invalid ticket!")
            return 1
